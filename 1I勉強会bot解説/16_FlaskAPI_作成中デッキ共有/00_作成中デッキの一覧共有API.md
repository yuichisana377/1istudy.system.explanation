...
- 実際に公開（[../14_FlaskAPI_CardMaker/01_一覧取得と保存API.md](../14_FlaskAPI_CardMaker/01_一覧取得と保存API.md)の`/save_cards`）されたら、対応するエントリはここから取り除かれます。
...
次は「お知らせ（notices）」のAPIを解説します。 → [../17_FlaskAPI_お知らせ/00_お知らせ一覧と詳細取得投稿API.md](../17_FlaskAPI_お知らせ/00_お知らせ一覧と詳細取得投稿API.md)
