...
| `screen-host-setup` | ホスト：セットアップ | タイトル・問題の作り方・途中参加可否などを設定。★ 2026/08/27〜、「詳細設定 →」ボタンで`screen-host-advanced`へ遷移する |
| `screen-host-advanced` | ホスト：詳細設定（★ 2026/08/27追加） | ボーナス問題（`hs-bonus-toggle`：なし/ランダム/最後の◯問、対象の問題は正解時の得点が2倍）・出題時間（`hs-time-limit`、5〜120秒）・正解発表の表示時間（`hs-reveal-duration`、2〜30秒）。SPA内の画面切り替えなので、host-setupへ戻っても入力値はそのまま保持される。今後の詳細設定もこの画面に追加していく想定 |
