...
- 上記どちらの特別なURLパラメータも無ければ、既存のセッションが保存されていないか確認します。あれば自動ログイン（3節）、無ければ`step-id`（ログインボタンだけの画面）を表示します。`session_token`の無い古い形式のセッションが残っていた場合は、この機会に消しておきます（パスワード方式が廃止された経緯にある、過去の名残データの掃除です）。
  > **★ 2026/09/05 追記**：`showStep("step-id")`の直前に、Discordのアプリ内ブラウザ
  > （DM・チャンネルのリンクをタップして開いた場合のブラウザ）かどうかをUser-Agentで
  > 判定する`isDiscordInAppBrowser()`（`/\bDiscord\//i.test(navigator.userAgent)`、
  > iOS/Android版Discordのアプリ内ブラウザに含まれる`"Discord/"`トークンを見る簡易判定）
  > を追加し、真であれば`loginWithDiscord()`を`showStep`より先に呼ぶようにした。
  > 背景：アプリ内ブラウザは端末の普段のブラウザとは`localStorage`を共有しない別の
  > ブラウザ環境として扱われるため、CardMaker等のリンクをDiscord上でタップするたびに
  > 保存済みセッションが見つからず、必ず`step-id`（ボタンだけの画面）まで来てしまっていた。
  > ただしアプリ内ブラウザを開けている時点で本人はDiscordにログイン済みであることが
  > 保証されているため、ボタンを待たず自動的にOAuth認可（`discord_login_start`）へ進んで
  > よい、という判断（Discord側で過去に一度でも本アプリを認可済みなら、認可確認画面すら
  > 出ずそのまま戻ってくることが多く、体感上ほぼ自動ログインになる）。UA判定に失敗しても
  > `step-id`はそのまま表示されるので、従来通りボタン操作でログインできる（実害が無い
  > フェイルセーフ設計）。
