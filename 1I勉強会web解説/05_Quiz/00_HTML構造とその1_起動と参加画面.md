...
| `screen-host-setup` | ホスト：セットアップ | タイトル・問題の作り方・途中参加可否などを設定。★ 2026/08/27〜、折りたたみ式の「詳細設定」（`hs-advanced-panel`）にボーナス問題（`hs-bonus-toggle`：なし/ランダム/最後の◯問、対象の問題は正解時の得点が2倍）を追加 |
