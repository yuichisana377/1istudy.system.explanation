...
対象：`bot.py`の「システム運用ログ」セクション。これは[../../1I勉強会web解説/08_ServiceInfo/00_ServiceInfo解説.md](../../1I勉強会web解説/08_ServiceInfo/00_ServiceInfo解説.md)で紹介した「サービス情報」ページの「運用ログ」タブが表示している、**GitHubのコミット履歴のような差分付きログ**を、サーバー側で実際に記録している部分です。
...
次は、この`log_event`/`file_diff`を実際に使う最初の例として、サーバー設定ファイル（`config_{guild_id}.json`）まわりの読み書きと、投稿内容の不正文字チェックの仕組みを解説します。 → [02_設定ファイルと不正文字チェック.md](02_設定ファイルと不正文字チェック.md)
