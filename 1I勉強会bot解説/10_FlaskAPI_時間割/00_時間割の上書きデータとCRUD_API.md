...
対象：`bot.py`の「Flask API — 時間割」セクション前半（時間割の「上書き」データ）。[[../../1I勉強会web解説/03_Timetable]]で解説した時間割ページの、サーバー側の実装です。
...
次は、学期ごとの基本時間割（曜日×時限の科目表そのもの）を管理するAPIを解説します。 → [[01_学期の基本時間割API.md]]
